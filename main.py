...
parser = argparse.ArgumentParser(
    description="Pipeline de classification Machine Learning avec MLflow"
)
parser.add_argument(
    "--data",
    required=True,
    help="Chemin du fichier CSV, ex: /home/chaima/ml_project_chaimatlili/merged_data.csv",
)
parser.add_argument(
    "--target", required=True, help="Nom de la colonne cible, ex: Churn"
)
parser.add_argument(
    "--model",
    required=True,
    help="Chemin pour sauvegarder/charger le modèle, ex: model.pkl",
)
parser.add_argument(
    "--action",
    required=True,
    choices=["train", "evaluate"],
    help="Action à effectuer (train/evaluate)",
)
parser.add_argument(
    "--test_size",
    type=float,
    default=0.2,
    help="Proportion des données de test (par défaut: 0.2)",
)
parser.add_argument(
    "--random_state",
    type=int,
